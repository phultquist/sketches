...
void draw() {
  ...
  int interestedTill = 500;
  float topValue = 1.1*pow(interestedTill, 0.5)+10;
  ...
  for (int i = 1; i <= interestedTill; i++) {
    ...
    float val = sim(i);
    float y = (height-2*buffer)-map(val, 0, topValue, 0, height-2*buffer);
...
float sim(int steps) { //numruns is the number of trials //steps is the number of steps per trial
  ...
  float avg = calculateAverage(distances);
